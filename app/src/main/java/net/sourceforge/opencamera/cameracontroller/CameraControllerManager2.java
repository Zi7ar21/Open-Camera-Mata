...
public class CameraControllerManager2 extends CameraControllerManager {
    ...
    private static final String[] fixedIds;
    static {
        final String product = Build.PRODUCT;
        if(product.equals("mata") || product.equals("NB1")) {
            fixedIds = new String[]{"0", "1", "2"};
        } else {
            fixedIds = null;
        }
    }

    public static String[] getCameraIdList(CameraManager manager) throws CameraAccessException {
        if(fixedIds != null)
            return fixedIds;
    ...
    public int getNumberOfCameras() {
        ...
        try {
            return getCameraIdList(manager).length;
        }
        catch(Throwable e) {
    ...
    public CameraController.Facing getFacing(int cameraId) {
        CameraManager manager = (CameraManager)context.getSystemService(Context.CAMERA_SERVICE);
        try {
            String cameraIdS = getCameraIdList(manager)[cameraId];
